...
export const CTA: React.FC<CTAProps> = ({ cta, className }) => {
  const safeCta: any = cta ?? { enabled: false };
  ...
  const backgroundImageUrl = useMemo(() => {
    ...
    if (safeCta?.backgroundImage) {
      imagePath = safeCta.backgroundImage;
    } else if (safeCta?.mediaItems && safeCta.mediaItems.length > 0) {
      imagePath = safeCta.mediaItems[0].url;
    } else if (safeCta?.image) {
      imagePath = safeCta.image;
    }
    
    return imagePath ? getImageSrc(imagePath) : null;
  }, [safeCta?.backgroundImage, safeCta?.mediaItems, safeCta?.image]);
  ...
  if (!safeCta?.enabled && !safeCta?.title) return null;
          ...
          {(safeCta.ctaButton || safeCta.primaryButton) && (
            <div className={cn(
              ...
              <a
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.backgroundColor = themeColors.hoverActive || themeColors.primaryButton;
                }}
                onMouseLeave={(e) => {
